...
def test_shell_representation_learning_path_selection_opens_lessons(monkeypatch) -> None:
    """Selecting the Level 3 representation path should open its ordered lessons."""
    monkeypatch.setenv("SDL_VIDEODRIVER", "dummy")
    app = UnifiedAppShell(settings=AppSettings(resolution=(640, 360)))
    ...
    try:
        ...
        assert app.screen_name == ScreenName.LESSONS
        assert app.selected_learning_path == path
        assert lessons[0].demo_id == "pca_lab"
        assert lessons[1].demo_id == "tsne_umap_exploration_lab"
        assert lessons[-1].demo_id == "time_series_forecasting_lab"
        assert app.selected_index == 0
    finally:
        pygame.quit()
