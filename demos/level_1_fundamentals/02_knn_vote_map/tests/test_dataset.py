"""Tests for synthetic classification dataset generation."""
...
def test_synthetic_classification_dataset_has_expected_shape() -> None:
    """Generated features and targets should have expected shapes."""
    config = SyntheticClassificationConfig(samples_per_class=SAMPLES_PER_CLASS)
    dataset = make_synthetic_classification_dataset(config)
...
def test_synthetic_classification_dataset_is_reproducible_for_same_seed() -> None:
    ...
    first = make_synthetic_classification_dataset(config)
    second = make_synthetic_classification_dataset(config)

    np.testing.assert_allclose(first.features, second.features)
    np.testing.assert_array_equal(first.targets, second.targets)


def test_synthetic_classification_dataset_contains_two_balanced_classes() -> None:
...
def test_synthetic_classification_dataset_has_expected_centers_without_noise() -> None:
...
def test_synthetic_classification_dataset_stores_metadata() -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            SyntheticClassificationConfig(samples_per_class=0),
            "samples_per_class must be greater than 0",
        ),
        (
            SyntheticClassificationConfig(class_distance=0.0),
            "class_distance must be greater than 0",
        ),
        (
            SyntheticClassificationConfig(noise_std=-1.0),
            "noise_std cannot be negative",
        ),
    ],
)
def test_synthetic_classification_dataset_rejects_invalid_config(
    config: SyntheticClassificationConfig,
    expected_message: str,
) -> None:
    """Invalid dataset configuration should fail clearly."""
    with pytest.raises(ValueError, match=expected_message):
        make_synthetic_classification_dataset(config)
